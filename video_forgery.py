...
if __name__ == '__main__':
    ...
    paths_to_video_paths = "C:\\Users\\iskan\\PycharmProjects\\VideoForgeryDetection\\utils\\video_paths"
    no_of_forgery = []
    video_name = []
    video_paths_collector = VideoPathsCollector(paths_to_video_paths)
    videos_paths = video_paths_collector.collect()
    all_manipulations = {}
    excel_writer = ExcelWriter("C:\\Users\\iskan\\PycharmProjects\\VideoForgeryDetection\\results", 'results')
    for video_path in videos_paths:
        file = get_video_name(video_path)
        if file == 'small_walkers.mp4':
            print('Process ', file)
